...
function miniCompile(ctx) {
	...
	if (ctx.data.weights[arr.length-1] !== ctx.data.sampleQuantity) throw Error('CompileError: weighting mismatch')
...
function compile() {
	...
	if (this.data.weights[len-1] !== this.data.sampleQuantity) throw Error('CompileError: weighting mismatch')
	arr.length = len
	this.data.isCompiled = true
	return this
}
...
function insert(v) {
	if (Array.isArray(v)) for (var i=0; i<v.length; ++i) this.data.arr.push([v[i], 1])
