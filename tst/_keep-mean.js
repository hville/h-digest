...
var KM = require('../src/_keep-mean')
var createWeighting = require('../src/weighting')
var E = require('../src/average')
var LS = require('lazy-stats')

function km(n) {
	return new KM(createWeighting(n))
}
...
c('keep average - constant', function() {
	var hd = km(6)
	for (var i=0; i<9; ++i) {
		hd.push(4)
		c('===', E(hd.values, hd.ranks), 4, hd.ranks)
	}
})
c('keep average - increasing', function() {
	var hd = km(5),
			vs = seq(9)
	for (var i=0; i<9; ++i) {
		hd.push(vs[i])
		c('===', E(hd.values, hd.ranks), i/2, `vs[i]:${vs[i]}, rs:${hd.ranks}`)
	}
})
c('keep average - decreasing', function() {
	var hd = km(5),
			vs = seq(9).reverse()
	for (var i=0; i<9; ++i) {
		hd.push(vs[i])
		c('===', E(hd.values, hd.ranks), 8-i/2, hd.ranks)
	}
})
...
c('uniform random, m=9, n=1000', function() {
	...
	c('<=', Math.abs(hd.quantile(0.5) - rnd[(n+1)/2]), 0.025, hd.ranks)//
